...
export default class AllinServer implements Party.Server {
  ...
  private startTurn() {
    ...
    // 컨트롤러(워든) 패시브 — 상대 턴 시작마다 상대 손패 1장을 이번 턴 사용 불가로.
    //   current = 이번 턴을 시작하는 플레이어, opponent = 그의 상대
    //   opponent 가 컨트롤러이면 current 의 카드를 1장 침묵 (상대 턴에 발동)
    if (opponent.className === "warden" && current.hand.length > 0) {
      ...
      const wardenId = this.getConnectionIdOf(opponent);
      if (wardenId) {
        this.sendToId(wardenId, {
          type: "passive_silence",
          cardId: silenced.id,
          cardName: silenced.name,
        });
      }
    }
